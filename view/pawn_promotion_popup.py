...
class PromotionOption(tkinter.Frame):
    ...
    '''
    def __init__(self, root, popup, piece, index: int):
        '''
        ...
        label.bind("<Button-1>", lambda *args: popup.update_selection(index))
...
class PawnPromotionPopup:
    ...
    '''
    def __init__(self, root, color: bool):
        '''
        ...
        tkinter.Button(self._popup, text="Promote", command=self._popup.destroy).grid(
            row=1, column=0, columnspan=len(piece_types), padx=5, pady=5, sticky=tkinter.E
        )

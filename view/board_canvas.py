...
# my code
from model import Coordinate, moves
from .pawn_promotion_popup import PawnPromotionPopup
from .view_utils import piece_to_image

SQUARE_SIZE = 78
SQUARE_OUTLINE = 2
TOTAL_SQUARE = SQUARE_SIZE + SQUARE_OUTLINE    # useful for the actual number of pixels in square
...
class BoardCanvas(tkinter.Canvas):
    ...
    '''
    def __init__(self, tk, theme, board):
        '''
        ...
        self._border_id = self.create_rectangle(BOARD_OUTLINE, BOARD_OUTLINE,
                            TOTAL_LENGTH - BOARD_OUTLINE, TOTAL_LENGTH - BOARD_OUTLINE, width=0)
    ...
    def _create_color_circle(self):
        ...
        self._color_circle = self.create_oval(circle_loc - CIRCLE_RADIUS, circle_loc - CIRCLE_RADIUS,
                            circle_loc + CIRCLE_RADIUS, circle_loc + CIRCLE_RADIUS, width=2)
        ...
        for row in range(1, 9):
            for col in range(1, 9):
                ...
                if piece is not None:
                    ...
                    self._images.append(image)   # save so it doesn't get garbage collected
    ...
    def on_left_click(self, event):
        ...
        if 1 <= row <= 8 and 1 <= col <= 8:
            ...
            if piece is not None and piece.is_white() == self._board.white_to_move():
                self._reset_squares()
                possible_moves = piece.get_moves(self._board, Coordinate.from_coords(row, col))
                ...
                if type(move) == moves.PawnPromotion:
                    ...
                    popup = PawnPromotionPopup(self._root_window, self._board.white_to_move())
    ...
    def _create_square(self, row: int, col: int):
        ...
        '''
        x, y = self._square_to_coords(row, col)
        return self.create_rectangle(x, y, x + SQUARE_SIZE, y + SQUARE_SIZE, width=SQUARE_OUTLINE)
    ...
    def _update_squares(self):
        ...
        for row in range(1, 9):
            for col in range(1, 9):
                square = self._squares[row - 1][col - 1]
                self.itemconfig(square.get_id(),
                                fill=self._theme.square_bg(row, col, focus=square.focus,
                                                            highlight=square.highlight),
                                outline=self._theme.square_outline(row, col, focus=square.focus,
                                                            highlight=square.highlight))

...
class Move(metaclass=abc.ABCMeta):
    ...
    def __init__(self, start: Coordinate, target: Coordinate):
        """
    ...
    @abc.abstractmethod
    def execute(self, board: BoardState) -> BoardState:
        """
    ...
    @abc.abstractmethod
    def was_pawn_advance(self) -> bool:
        """
    ...
    def to_notation(self, new_board: BoardState) -> str:
        """
        ...
        if new_board.is_checkmate():
            ...
            king_coord = new_board.find_king(new_board.white_to_move())
            assert king_coord is not None
            attackers = new_board.count_attackers(
                king_coord, not new_board.white_to_move()
            )
            if attackers == 1:
                notation += "+"
            elif attackers == 2:
    ...
    @abc.abstractmethod
    def basic_notation(self, new_board: BoardState) -> str:
        """
...
class BasicMove(Move):
    ...
    def execute(self, board: BoardState):
        if not self._start.is_valid():
            # invalid start position
            raise InvalidMoveException(
                f"Invalid move: bad start position {self._start}"
            )
        elif not self._target.is_valid():
            # invalid target position
            raise InvalidMoveException(
                f"Invalid move: bad target position {self._target}"
            )
        ...
        if target_piece is not None:
            if target_piece.is_white() == board.white_to_move():
                ...
                raise InvalidMoveException(
                    f"Invalid move: {self._start}-{self._target}"
                )
            else:
        ...
        if type(piece) == pieces.Pawn:
            ...
            if abs(self._start.row - self._target.row) == 2:
                ...
                board.set_en_passant(
                    Coordinate.from_coords(
    ...
    def basic_notation(self, new_board: BoardState) -> str:
        ...
        if piece_moved is None:
            raise Exception(f"No piece where expected ({self._target.to_notation()})")
        elif piece_moved.FEN_SYMBOL == "P":
            # Pawn move
...
class KingsideCastle(Move):
    ...
    def basic_notation(self, new_board: BoardState):
        return "0-0"
...
class PawnPromotion(BasicMove):
    ...
    def __init__(
        self, start: Coordinate, target: Coordinate, classinfo: Type[pieces.Piece]
    ):
        """
    ...
    def set_promotion(self, classinfo: Type[pieces.Piece]):
        """
    ...
    def execute(self, board: BoardState):
        ...
        piece = board.get_piece(self._start.row, self._start.col)
        if (
            not isinstance(piece, pieces.Pawn)
            or piece.is_white() != board.white_to_move()
        ):
...
class EnPassant(Move):
    ...
    def execute(self, board: BoardState):
        if (
            not (self._start.is_valid() and self._target.is_valid())
        ...
        if board.white_to_move():
            ...
            other_pawn_coords = Coordinate.from_coords(
                self._target.row - 1, self._target.col
            )
        else:
